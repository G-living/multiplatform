...
const Cart = (() => {
  ...
  function _load() {
    try {
      const raw = localStorage.getItem(GINORI_CONFIG.cart.storageKey);
  ...
  function _save() {
    try {
      localStorage.setItem(GINORI_CONFIG.cart.storageKey, JSON.stringify(_items));
    } catch (e) {
  ...
  function addItem(item) {
    ...
    if (existing) {
      existing.quantity = Math.min(
        ...
        GINORI_CONFIG.cart.maxQuantity
      );
    } else {
      _items.push({
        uid:         Utils.uid(),
        ...
        sku:         item.sku,
        comodin:     item.comodin || GINORI_CONFIG.images.placeholder,
        price:       item.price,
        quantity:    item.quantity || 1,
        image:       item.image || GINORI_CONFIG.images.placeholder,
      });
    }
    _save();
  ...
  function updateQuantity(uid, qty) {
    ...
    else { item.quantity = Math.min(q, GINORI_CONFIG.cart.maxQuantity); _save(); }
    _resetInactivityTimer();
  }
      ...
      <div class="cart-item" data-uid="${Utils.sanitize(item.uid)}">
        ...
        <div class="cart-item-image-wrap">
          <img
            ...
            loading="lazy"
            title="Ampliar"
            onerror="this.onerror=null;this.src='${GINORI_CONFIG.images.placeholder}'"
          >
        </div>
        ...
        <div class="cart-item-controls">
          <div class="cart-qty-selector">
            ...
            <input
              ...
              type="number" min="1" max="${GINORI_CONFIG.cart.maxQuantity}"
              value="${item.quantity}"
  ...
  function _checkPostPayment() {
    ...
    try { sessionStorage.removeItem('ginori_pending_order'); } catch {}
  }

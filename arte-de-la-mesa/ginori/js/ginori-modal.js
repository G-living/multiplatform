...
const Modal = (() => {
  ...
  function _renderProductModal(product) {
    ...
    if (mainImg) {
      ...
      mainImg.onerror = () => {
        ...
        mainImg.src = GINORI_CONFIG.images.placeholder;
      };
    }
  ...
  function _handleVariantClick(e) {
    ...
    if (action === 'dec') {
      ...
      _quantities[sku] = Math.min(GINORI_CONFIG.cart.maxQuantity, current + 1);
    }
  ...
  function _handleFamilyVariantClick(e) {
    ...
    _quantities[sku] = action === 'dec'
      ? Math.max(0, cur - 1)
      : Math.min(GINORI_CONFIG.cart.maxQuantity, cur + 1);
  ...
  function openZoom(src, alt = '') {
    ...
    if (img) {
      img.src = src || GINORI_CONFIG.images.placeholder;
      img.alt = alt;
      img.onerror = () => { img.onerror = null; img.src = GINORI_CONFIG.images.placeholder; };
    }
  ...
  // ═══════════════════════════════════════════════════
  const _DRAFT_KEY = 'ginori_checkout_draft';
  ...
  async function _handleSubmitWA(e) {
    ...
    const waUrl = `https://wa.me/${GINORI_CONFIG.brand.phone.replace('+','')}?text=${encodeURIComponent(msgTemp)}`;
  ...
  function _updateWompiTotals() {
    const cfg       = GINORI_CONFIG.checkout;
  ...
  async function _submitWompi(pct) {
    ...
    const _pendingPayload = JSON.stringify({
      ...
      campaniaId:       GINORI_CONFIG?.campania?.id  || '',
      catalogoId:       GINORI_CONFIG?.catalogo?.id  || '',
    });
    try { sessionStorage.setItem('ginori_pending_pedido', _pendingPayload); } catch(e) { Logger.warn('modal.js: sessionStorage write error', e); }
    try { localStorage.setItem('ginori_pending_pedido', _pendingPayload); }   catch(e) { Logger.warn('modal.js: localStorage write error', e); }
    ...
    try { sessionStorage.setItem('ginori_wompi_redirect', '1'); } catch(e) {}
  ...
  async function _submitConGiftCard() {
    ...
    const items    = Cart.getItems();
    const subtotal = _ckSubtotal;
    const cfg2     = GINORI_CONFIG.checkout;
    ...
    try { sessionStorage.setItem('ginori_pending_pedido', _pendingGiftPayload); } catch(e) {}
    try { localStorage.setItem('ginori_pending_pedido',   _pendingGiftPayload); } catch(e) {}
    ...
    window.location.href = `ginori-checkout.html?reference=${encodeURIComponent(reference)}&transaction_status=APPROVED&giftPaid=1`;
  }
  ...
  async function _handleSubmitGift(e) {
    ...
    try { sessionStorage.setItem('ginori_gift_payload', _giftPayloadStr); } catch(e) {}
    try { localStorage.setItem('ginori_gift_payload',   _giftPayloadStr); } catch(e) {}
    try { localStorage.setItem('ginori_gift_' + reference, _giftPayloadStr); } catch(e) {}
    ...
    try { sessionStorage.removeItem('ginori_wompi_redirect'); } catch(e) {}
    // Flag específico para Gift — localStorage persiste cross-origin; sessionStorage como backup
    try { sessionStorage.setItem('ginori_gift_redirect', reference); } catch(e) {}
    try { localStorage.setItem('ginori_gift_redirect', reference); }   catch(e) {}
    window.location.href = `${cfg.wompiCheckoutUrl}?${params.toString()}`;
  }
  ...
  function restoreGiftStep2() {
    ...
    try {
      const raw = localStorage.getItem('ginori_gift_payload')
               || sessionStorage.getItem('ginori_gift_payload');
...
window.addEventListener('pageshow', (e) => {
  ...
  try {
    const giftRef = sessionStorage.getItem('ginori_gift_redirect')
                 || localStorage.getItem('ginori_gift_redirect');
    if (giftRef) {
    ...
    if (sessionStorage.getItem('ginori_wompi_redirect') === '1') {
      sessionStorage.removeItem('ginori_wompi_redirect');

// @version    v21.1  @file ginori-config.js  @updated 2026-03-08  @session fix-paths
/* ===== GINORI V2 - config.js ===== */
...
const GINORI_CONFIG = {
  // Marca
  brand: {
    name: 'GINORI by G-Living | Italian Living & Design | Est. 2018',
    phone: '+573004257367',
  ...
  images: {
    ...
    comodinPath: 'images/comodines-ginori/',
  ...
  checkout: {
    ...
    signatureWorkerUrl: 'https://ginori-signature-generator.workers.dev',  // TODO: configurar worker GINORI
    sheetsUrl: '',  // TODO: configurar Google Apps Script GINORI
    checkoutUrl: 'https://g-living.github.io/multiplatform/arte-de-la-mesa/ginori/ginori-checkout.html',
...
const Utils = {
  ...
  calcPresale(precioCop) {
    ...
    const descPct   = GINORI_CONFIG.campania.descuentoPct || 0;
  ...
  formatPrice(price) {
    ...
    return GINORI_CONFIG.checkout.currencySymbol +
  ...
  buildWhatsAppUrl(message) {
    ...
    return `${GINORI_CONFIG.brand.whatsappBase}${GINORI_CONFIG.brand.phone}?text=${encoded}`;
  },
...
const ImageManager = {
  ...
  withFallback(src, imgEl) {
    const placeholder = GINORI_CONFIG.images.placeholder;
    if (imgEl) {
      imgEl.onerror = () => {
        imgEl.onerror = null;
   ...
   */
  productSrc(filename) {
    if (!filename) return GINORI_CONFIG.images.placeholder;
    ...
    return GINORI_CONFIG.images.productPath + filename;
  },
  ...
  comodinSrc(filename) {
    if (!filename) return GINORI_CONFIG.images.placeholder;
    if (filename.startsWith('http') || filename.startsWith('/')) return filename;
    return GINORI_CONFIG.images.comodinPath + filename;
  },
};
...
const Logger = {
  ...
  log(...args) { if (this._dev) console.log('[GINORI]', ...args); },
  warn(...args) { if (this._dev) console.warn('[GINORI]', ...args); },
  error(...args) { console.error('[GINORI]', ...args); },
};

// ===== EXPORTAR AL SCOPE GLOBAL =====
window.GINORI_CONFIG = GINORI_CONFIG;
window.Utils = Utils;
...
const Toast = (() => {
  ...
  function show(message, type = 'success', duration = null) {
    ...
    const dur = duration ?? GINORI_CONFIG.toast.duration;
...
const Campania = (() => {
   ...
   */
  function onReady(fn) {
    if (GINORI_CONFIG.campania.loaded) { fn(); return; }
    _callbacks.push(fn);
  }
  ...
  function descuentoPct() {
    return GINORI_CONFIG.campania.descuentoPct || 0;
  }
  ...
  async function cargar() {
    try {
      ...
      const url = GINORI_CONFIG.checkout.sheetsUrl +
        '?action=getCampanias';
      ...
      if (data.ok && Array.isArray(data.campanias) && data.campanias.length > 0) {
        ...
        const match = data.campanias.find(
          c => c['Campaña_ID'] === GINORI_CONFIG.campania.id
        );
        if (match && Number(match['Descuento_Pct']) > 0) {
          GINORI_CONFIG.campania.descuentoPct = Number(match['Descuento_Pct']);
          Logger.log(`Campania: "${GINORI_CONFIG.campania.id}" activa — ${GINORI_CONFIG.campania.descuentoPct}% dto.`);
        } else {
      ...
      GINORI_CONFIG.campania.loaded = true;

 ...
 *      Lee mlg_pending_pedido → confirmarPagoWompi(ref, status, txId, payload)
 *   B) Compra de Gift Card con Wompi (reference empieza con GIFT-):
 *      Lee mlg_gift_payload → createGiftCard(payload) → confirmarPagoWompi(ref, APPROVED, txId)
 *   C) Producto pagado 100% con Gift Card (giftPaid=1):
 *      Lee mlg_pending_pedido (con _giftPaid:true) → confirmarPagoWompi(ref, APPROVED, GIFT_CARD, payload)
...
const _checkoutGiftStorage = (() => { try { return sessionStorage.getItem('mlg_gift_redirect') || localStorage.getItem('mlg_gift_redirect') || ''; } catch(e) { return ''; } })();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (closeBtn) {
    closeBtn.addEventListener('click', () => { window.location.href = 'mlg-index.html'; });
  }
  ...
  if (!status && !txId) { window.location.replace('mlg-index.html'); return; }
...
function _handleStatus(status, reference, txId, isGiftCard, giftPaid) {
  ...
  const btnCatalogo   = '<a href="mlg-index.html" class="btn btn-primary">Explorar catálogo</a>';
  const btnReintentar = '<a href="mlg-index.html" class="btn btn-secondary">Volver al catálogo</a>';
  const btnWA         = `<a href="https://wa.me/${(typeof MLG_CONFIG !== 'undefined' ? MLG_CONFIG.brand.phone.replace('+','') : '573004257367')}" target="_blank" class="btn btn-secondary">Escríbenos por WhatsApp</a>`;

  if (status === 'APPROVED') {
    // Vaciar carrito
    try { localStorage.removeItem(MLG_CONFIG.cart.storageKey); } catch(e) {}
    ...
    if (isGiftCard && !giftPaid) {
      ...
      try {
        const raw = sessionStorage.getItem('mlg_gift_payload')
                 || localStorage.getItem('mlg_gift_payload')
                 || localStorage.getItem('mlg_gift_' + reference);
        if (raw) giftPayload = JSON.parse(raw);
      } catch(e) { console.warn('checkout.js: error leyendo gift payload', e); }
      try { sessionStorage.removeItem('mlg_gift_payload'); }     catch(e) {}
      try { localStorage.removeItem('mlg_gift_payload'); }       catch(e) {}
      try { localStorage.removeItem('mlg_gift_' + reference); } catch(e) {}
        ...
        `${emisorNombre ? `<p style="margin:0 0 12px">¡Gracias, <strong>${emisorNombre}</strong>! Tu regalo está activado y listo para sorprender. ✨</p>` : '<p style="margin:0 0 12px">¡Gracias por este gesto tan especial! Tu regalo está activado y listo para sorprender. ✨</p>'}
         ...
         <div style="background:#f9f6f0;border-radius:8px;padding:14px;margin:14px 0;font-size:13px;text-align:left">
           ...
           <ol style="margin:0;padding-left:18px;color:#555;line-height:1.7">
             <li>El destinatario visita <strong>mlg-index.html</strong> y elige su pieza.</li>
         ...
         <p style="margin:12px 0 0;font-style:italic;color:#888;font-size:13px">Cada pieza es única, importada directamente. Un regalo que perdura.</p>`,
        reference,
        btnCatalogo + btnWA,
    ...
    if (giftPaid) {
      ...
      try {
        const raw = sessionStorage.getItem('mlg_pending_pedido')
                 || localStorage.getItem('mlg_pending_pedido');
        if (raw) pedidoPayload = JSON.parse(raw);
      } catch(e) { console.warn('checkout.js: error leyendo payload gift paid', e); }
      try { sessionStorage.removeItem('mlg_pending_pedido'); } catch(e) {}
      try { localStorage.removeItem('mlg_pending_pedido'); }   catch(e) {}
      ...
      setContent(
        ...
        `¡Qué alegría tenerte de vuelta! Tu pedido ha sido confirmado y ya estamos trabajando en él con todo el cariño.<br><br>
         ...
         <em>Cada pieza que elegiste es única — importada directamente, especialmente para ti.</em>`,
        reference,
        btnCatalogo + btnWA,
        'confirm-title--success'
      );
    ...
    setContent(
      '🏺', '¡Gracias por tu compra!',
      `¡Qué alegría! Tu pago fue confirmado exitosamente — cada pieza que elegiste es única, importada directamente especialmente para ti.<br><br>
       ${_bonoLineA}
       En los próximos minutos recibirás un <strong>email de confirmación</strong> con el detalle completo de tu pedido.<br><br>
       Nuestro equipo se pondrá en contacto contigo para coordinar la entrega.<br><br>
    ...
    if (isGiftCard) {
      const btnRetryGift = `<a href="mlg-index.html?retryGift=1" class="btn btn-primary">Reintentar pago</a>`;
      setContent(
        '✗', 'Pago no completado',
      ...
      const btnRetryWompi = `<a href="mlg-index.html?retryWompi=1" class="btn btn-primary">Intentar nuevamente</a>`;
      setContent(
        '✗', 'Pago no procesado',

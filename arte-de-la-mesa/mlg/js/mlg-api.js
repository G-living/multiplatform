// @version    v1.0  @file mlg-api.js  @updated 2026-03-11  @session mlg-catalog
/* ===== MLG V1 - api.js =====
...
const Api = (() => {
  ...
  function _url() {
    return MLG_CONFIG?.checkout?.sheetsUrl || '';
  }
  ...
  async function _post(payload) {
    ...
    try {
      const secured = {
        _token: MLG_CONFIG?.checkout?.apiToken || '',
        ...payload,
      };
  ...
  async function _get(params) {
    const url = _url();
    if (!url || url.includes('PENDING')) return { ok: false, error: 'sheetsUrl no configurada' };
    try {
  ...
  function _campaniaId() {
    return MLG_CONFIG?.campania?.id || '';
  }

  function _catalogoId() {
    return MLG_CONFIG?.catalogo?.id || '';
  }
  ...
  async function cancelGiftCard(referencia) {
    return _post({ action: 'cancelGiftCard', referencia });
  }

...
const CFG = {
  ...
  NOMBRE_TIENDA   : 'MLG by G-Living | Italian Living & Design | Est. 2018',
  ...
  CATALOGO        : 'https://g-living.github.io/multiplatform/arte-de-la-mesa/mlg/mlg-index.html',
...
function _emailIdentidadSospechosa(b, rowEncontrada) {
  try {
    const subject = 'ALERTA IDENTIDAD SOSPECHOSA — MLG — REVISAR DE INMEDIATO';
    const body = `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto">
  ...
  <div style="background:#fff8f8;padding:24px;border:2px solid #8b0000;border-top:none">
    ...
    <p style="font-size:12px;color:#aaa;margin-top:16px">
      Generado automáticamente por MLG Backend · ${new Date().toISOString()}
    </p>
  </div>
</div>`;
...
function _emailInfluencerVenta(inflCodigo, opts) {
  ...
  try {
    ...
    const body = _emailWrapper(inflNombre, `
      ...
      <p style="font-size:14px;margin:12px 0 0">¡Gracias por llevar MLG a tu comunidad! Cada recomendación tuya pone piezas únicas en nuevas mesas.</p>
    `);
...
function setupProtections() {
  ...
  _protectCols(CFG.SHEETS.CLIENTES,
    ...
    'Sistema MLG — no editar manualmente'
  );
...
function emailResumenHuerfanos() {
  ...
  const asunto = cumpleHoy.length > 0 && totalAlerts === 0
    ? `[MLG] 🎂 ${cumpleHoy.length} cumpleaños hoy — ${fechaStr}`
    : totalAlerts === 0
    ? `[MLG] Sin pendientes — ${fechaStr}`
    : cumpleHoy.length > 0
    ? `[MLG] 🎂 ${cumpleHoy.length} cumpleaños · ⚠️ ${totalAlerts} pendiente(s) — ${fechaStr}`
    : `[MLG] ⚠️ ${totalAlerts} pendiente(s) — ${fechaStr}`;
...
<div style="font-family:Georgia,serif;max-width:680px;margin:auto;color:#1a1610">
  ...
  <div style="background:#1a1610;padding:10px 28px;border-radius:0 0 8px 8px;text-align:center">
    <p style="color:#888;font-size:10px;margin:0">
      Generado automáticamente · MLG v1.0 · <a href="${urlSheet}" style="color:#C4A05A">Abrir Sheets</a>
    </p>
  </div>
</div>`;
